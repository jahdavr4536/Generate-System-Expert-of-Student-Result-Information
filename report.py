...
class reportClass:
    def __init__(self,root):
        ...
        #********* title *********
        title = Label(self.root,text="View Student Result",font = ("goudy old style",20,"bold"),bg="orange",fg="#262626").place(x=15,y=15,width=1180,height=50)
        
        #********* Label *********
        self.var_search=StringVar()
        self.var_id=""

        lbl_search=Label(self.root,text="Search By Roll No.",font=("goudy old style",20,"bold"),bg="white").place(x=250,y=100)
        txt_search=Entry(self.root,textvariable=self.var_search,font=("goudy old style",20),bg="lightyellow").place(x=480,y=100,width=150)
        btn_search=Button(self.root,text="Search",font=("goudy old style",15,"bold"),bg="#03a9f4",fg="white",cursor="hand2",command=self.search).place(x=670,y=100,width=100,height=35)
    ...
    def delete(self):
        ...
        try:
            if self.var_id=="":
                messagebox.showerror("Error","First search student result",parent=self.root)
            else:
